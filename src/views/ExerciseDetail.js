...
const list = [
  {
    id : 1,
    title: "Quick Streching Sequence and the United States of America",
    ...
    <Table data = {data} className = "Table1" >{(tableList) => (
      ...
      <Header style = {{backgroundColor: '#012169'}}>
        <HeaderRow style = {{backgroundColor: '#012169'}}>
          ...
          <HeaderCell style = {{height: "50px", paddingLeft: "90px"}}> Time </HeaderCell>
        </HeaderRow>
      </Header>
      <Body>
        {tableList.map((item) => (
          <Row key = {item.id} item = {item} style = {{backgroundColor: 'transparent'}}>           
            <Cell>
              <img src  = {item.imgUrl} style={{ width: '150px', height: '100px', marginLeft: '10px', marginBottom: '10px', marginTop:'15px' }}/>
            </Cell>
            <Cell >
            ...
            <Cell className='time' style = {{paddingLeft: "100px"}}>{item.description}</Cell>        
          </Row>
        ))}
      </Body>
      </>
